...
namespace RallyGame.UI
{
    ...
    public class DealerView : MonoBehaviour
    {
        ...
        private void OnDisable() { if (onStockChanged) onStockChanged.Unregister(Rebuild); }
        ...
        {
            var def = database.GetPart(item.definitionId);
            if (def != null && economy.BuyPart(def, item.condition, item.price)) { item.sold = true; Rebuild(); }
        }
